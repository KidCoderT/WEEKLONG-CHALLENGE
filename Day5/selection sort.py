...
window = pyglet.window.Window(width=700, height=700, caption="SELECTION SORT!")
...
def update(dt):
	...
	if is_sorted(l):
		...
		min = l[current_index]
		for i in range(current_index+1, size):
			if l[i] < min:
				min_index = i
				min = l[i]

		temp = l[current_index]
		l[current_index] = l[min_index]
		l[min_index] = temp
		current_index += 1
